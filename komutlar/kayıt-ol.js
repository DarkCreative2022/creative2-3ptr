...
let prefix = await db.fetch(`prefix.${message.guild.id}`) || fynx.prefix  
  ...
  let isim = args[0]
  let yas = args[1];
  if (!isim) return message.channel.send(`Seni Kayıt Etmem İçin Bir İsim Girmelisin : \`${prefix}kayıt Arda\``)
  ...
  if (!yas) return message.channel.send(`Seni Kayıt Etmem İçin Bir Yaş Girmelisin : \`${prefix}kayıt Ayşe 17\``)
