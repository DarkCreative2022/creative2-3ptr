...
if (!args[0])  {
  ...
  if (args [0] == 'aç') {
    db.set(`reklamengel_${message.guild.id}`, '<:blue_tick_1:915575001787289642> **Küfür Engel Aktif!**')
    let lus = await db.fetch(`kufurE_${message.guild.id}`)
    
    const reklamengelcim = new Discord.MessageEmbed()
    .setTitle('Başarılı')
      .setColor("#6278c5")
    .setDescription('<:blue_tick_1:915575001787289642> **Küfür Engel Başarıyla Açıldı**')
  ...
  if (args [0] == 'kapat') {
   ...
   const küfürengelcim22 = new Discord.MessageEmbed()
    .setTitle('Başarılı')
    .setDescription('<:blue_tick_1:915575001787289642> **Küfür Engel Başarıyla Kapatıldı**')
