...
.setDescription(`<:ItemMusicNote:915553560358420510> **| Müzik oynatabilmek için bir ses kanalında olmanız gerekmektedir!**`)
...
.setDescription(`<:ItemMusicNote:915553560358420510> **| Müzik oynatabilmek için aranacak kelime veyahut kelimeler giriniz!**`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2022`, client.user.avatarURL());    
...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Creative Bot | Kuyruğa Ekle")
  .setDescription(`<:ItemMusicNote:915553560358420510> **|** \`${sarki.name}\` **adlı müzik,**\n${message.author} **tarafından kuyruğa eklendi!**`)
.setThumbnail(`https://drummofficial.com/wp-content/uploads/2017/08/equalizer3.gif`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2022`, client.user.avatarURL());   
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Creative Bot | Oynatılan Şarkı")
  .setDescription(`<:ItemMusicNote:915553560358420510> **|** \`${sarki.name}\` **adlı müzik şu anda oynatılıyor.**`)
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Creative Bot")
  .setDescription(`<:ItemMusicNote:915553560358420510> **| Kuyruktaki tüm müzikler oynatıldı. Creative Bot kanaldan ayrılıyor.\n\nCreative Bot Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2022`, client.user.avatarURL());  
